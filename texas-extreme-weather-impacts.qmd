...
```
Read in the Texas geomoetry data and Texas socioeconomic data.
```{r}
#| output: false
...
#### Create a dataframe that includes socioeconomic data for the only the homes that experienced blackouts
```{r}
...
```
#### Create a data frame that identifies whether the home did or did not experience a blackout
```{r}
...
```{r}
# census <- if (census$blackedout == TRUE){
#   mutate(blackedout_yn = "Blackout")
# } else {
#   mutate(blackedout_yn = "No Blackout")
# }
```

```{r}
# create histograms of incomes between those that experienced blackout and those that did not
 ggplot(census, aes(B19013e1))+
  ...
  facet_wrap(~ blackedout, 
             ncol = 2, 
             scales = "free_x",
            # labeller = as_labeller(c(True = "Blackout",
            #                          False = "No blackout")),
...
```{r}
# create boxplots of incomes between those that experienced blackout and those that did not
