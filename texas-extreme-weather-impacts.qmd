...
## Functions to Check and Change CRS 
...
texas_crs_change <- function(df1){
  df1 <-  st_transform(df1, crs=st_crs('EPSG:3083'))
}
```
...
# texas_crs_change(texas_socioeconomics)

```
```{r}
st_crs(blackedout_homes)
```
