...
blackout_map <- tm_basemap("Esri.OceanBasemap")+
  tm_shape(blackout) + 
  tm_raster(palette = c('black', 
                        '#58544C',
                        'darkgrey', 
...
census_map <- tm_shape(census) + 
  tm_polygons() +
  tm_add_legend(labels = c("Tracts That Did Not Lose Power"),
                 fill = "grey",
              type = 'polygons') +
...
# create histograms of incomes between those census tracts that experienced blackouts and those that did not
 ggplot(census, aes(B19013e1))+
  ...
  labs(x = "Income - USD ($)",
       y = "Frequency",
       title = "USD($) Income Level for Homes that Experienced Blackouts")+
  theme_minimal() +
...
ggplot(census, aes(y = B19013e1))+
  ...
  labs(x = "Income",
       y = "Income - USD ($)",
       title = "Median USD($) Income Level for Homes and Their Blackout Experience") +
  theme_minimal() +
