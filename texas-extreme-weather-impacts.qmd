...
texas_socioeconomics <- full_join(texas, texas_income, by = "GEOID_Data")
```
...
all_homes <- houses |> 
  mutate(blackout = osm_id %in% blackedout_homes$osm_id)
  
# subset data only data that overlaps with blackedout homes
blackedout_socioecon <- st_intersection(texas_socioeconomics, all_homes) 

# select only necessary columns
all_blackedout_socioecon <- blackedout_socioecon[, c("GEOID_Data", "TRACTCE", "B19013e1", "blackout")]
```
...
```{r}
 ggplot(all_blackedout_socioecon, aes(B19013e1))+
  geom_histogram(color = 'Black', fill = "#F4F1BB") +
  labs(x = "Income",
       y = "Frequency",
       title = "Income Level for Homes that Experienced Blackouts")+
  theme_minimal() +
  facet_wrap(~blackout) # unique on blackout 
```
```{r}
ggplot(all_blackedout_socioecon, aes(y = B19013e1))+
  geom_boxplot(color = 'Black') +
  labs(x = "Income",
       y = "Income",
       title = "Median Income Level for Homes and Their Blackout Experience")+
  theme_minimal() +
  facet_wrap(~blackout, strip.position = "bottom") +
