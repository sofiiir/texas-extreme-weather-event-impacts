...
Look at the type of geometries
```{r}
st_geometry(blackout)
```
...
```
Since we'll be plotting with both blackout data and the original light data, the crs for the light data must also be changed.
```{r}
light_0207_nad <- st_transform(light_0207, crs = st_crs('NAD83'))
light_0216_nad <- st_transform(light_0207, crs = st_crs('NAD83'))
```
...
```{r}
# WHY WOULD I WANT TO ST_UNION?
# ALL OF MY VALUES BECOME INVALID WHEN I USE st_union
...
```{r}
# crs must be the same for st_difference to work
# change crs of blackout data
blackout <- st_transform(blackout, crs = st_crs('EPSG:3857'))

true_blackout <- st_difference(blackout, exc_highway)
```
